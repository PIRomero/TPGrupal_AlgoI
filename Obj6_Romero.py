...
def consultar_mensajes():
    # Muestra los mensajes recibidos ya decodificados
    ...
    for linea in reversed(lineas):
        ...
        if len(partes) != 4:
            continue
        destinatario, remitente, cifrado, mensaje_cifrado = partes

        if destinatario != "*" and destinatario != usuario_actual:
            continue

        if cifrado.startswith("C"):
            # Descifra el mensaje haciendo el cifrado en reversa
            clave = int(cifrado[1:])
            mensaje_descifrado = cifrar_cesar(mensaje_cifrado, -clave)
        elif cifrado.startswith("A"):
            mensaje_descifrado = cifrar_atbash(mensaje_cifrado)
        else:
            mensaje_descifrado = mensaje_cifrado

        if destinatario == "*":
            remitente = "#" + remitente

        mensajes.append(f"{remitente}: {mensaje_descifrado}")

    ventana_mensajes = Toplevel()
    ventana_mensajes.title("Mensajes Recibidos")
    Label(ventana_mensajes, text="Lista de Mensajes:").pack(pady=10)

    if len(mensajes) == 0:
        Label(ventana_mensajes, text="No hay mensajes recibidos").pack(pady=10)
    else:
        cuadro = Text(ventana_mensajes, width=60, height=20)
        cuadro.pack()
        for m in mensajes:
            cuadro.insert("end", "—" * 70 + "\n")
            cuadro.insert("end", m + "\n")
        cuadro.insert("end", "—" * 70 + "\n")
        cuadro.insert("end", f"Total de mensajes: {len(mensajes)}")
...
def abrir_ventana_principal():
    ...
    tk.Button(btn_frame, text="Consultar mensajes recibidos", width=18, command=consultar_mensajes).grid(row=3, column=0, padx=5, pady=10)
